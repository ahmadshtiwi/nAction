...
import { WorkLog, WorkLogFilter, WorkLogTask } from './log.model';
...
@Component({
  selector: 'app-log',
  imports: [TableComponent, AutoFormErrorDirective, CommonModule, FormsModule, NgbDropdownModule, NgSelectModule, MaxValueDirective, RestrictNonNumericDirective, AttachmnetComponent],
  templateUrl: './log.component.html',
  styleUrl: './log.component.scss'
})
export class LogComponent implements OnInit {
  @ViewChild('content') content!: TemplateRef<any>;
  tableHeaderTitle = 'WorkLogs';
  tableHeaderButton = 'Add Item';
  tableHeadersDaily = ['Resource Name', 'Date', 'From', 'To', 'Project initial', 'Task Name', 'Description', 'Completed', 'Logs'];
  ...
  constructor(private offcanvasService: NgbOffcanvas, private loginService: LoginService, private lookupService: LookUpsService, private userService: UsersService,
    private WorkLogService: LogService, private route: ActivatedRoute, private modalService: NgbModal, private spinnerService: SpinnerService, private location : Location) {
  ...
  getFilters() {
    this.columnFilters = {
      ...
      'Project initial': { isText: false, isDropdown: true, isDates: false, options: this.transformedProjects },
  ...
  filterData() {
    ...
    this.pagination.projectId = this.filters['Project initial'] ? this.filters['Project initial'] : null;
  ...
  displayColumns1 = (data: WorkLog) => {
    return [
      ...
      { isText: true, text: data.assignmentTitle },
      { isText: true, text: data.description },
      { isText: true, text: data.percentageCompleted.toString() + ' %' },
    ];
  };
